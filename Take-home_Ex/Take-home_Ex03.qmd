...
Take-home exercise 4 showcases the prototyping for one module in our Visual Analytics group project: "Track Singapore Public Dollars!" Taxpayers want to know where and how the government spends money on developing the country. While Singapore government procurement data is publicly available, with over 6,000 entries per year (on average), it's difficult for the public to extract meaningful insights. Our visualization tool aims to democratize this dataset and make it accessible to the public.

This assignment is divided into 2 segments (webpages):

1\. Initial exploratory analysis (primarily to understand the dataset structure): [Here](https://cathyc.netlify.app/Take-home_Ex/Take-home_Ex03_2.html)

2\. Prototype: provides the various visualisation options and a storyboard for the **Network Insights Module** (this page)

## 2. R Packages Evaluation

| Purpose | R package | Description |
|-----------------|-----------------|--------------------------------------|
| Network visualisation | igraph | A package for network analysis and visualization |
|  | visNetwork | An R interface to the 'vis.js' JavaScript library for interactive network visualization |
| Other visualisation | ggplot2 | A powerful data visualization package based on the grammar of graphics |
|  | ggraph | An extension of ggplot2 for creating network/graph visualizations |
| Data manipulation | tidyverse | A collection of R packages designed for data science that share a common philosophy and design |
| Shiny components: | shiny | A web application framework for R to create interactive web applications |
|  | shinydashboard | An extension of Shiny that provides components for creating dashboards |
|  | shinyjs | Extends Shiny applications with JavaScript operations |
| Interactive tables | DT | Provides an R interface to the JavaScript library DataTables for interactive data tables |
| Date handling | lubridate | Provides functions for parsing, manipulating, and performing calculations with date-time data. |
|  | rlang | A toolkit for working with core language features of R and the tidyverse |
...
Before joining the datasets, we conducted a quick exploratory data analysis to determine whether the data would need further processing before or after merging. Please see section 3.2 for the preliminary EDA.
...
In the earlier section, we imported two datasets. One dataset contains derived columns such as tender category (from machine learning) and agency type (from government definitions). In this step, we will join these datasets to incorporate these derived columns for use in our Network Analysis.
...
The EDA is parked in a [different page for reference](https://cathyc.netlify.app/Take-home_Ex/Take-home_Ex03_2.html). The purpose is to understand the general information of this dataset to process data cleaning and structuring.

## 4 Module design

### 4.1 Design thinking

I discovered that our network has a directional structure, where the "from" node is always an agency and the "to" node is always a supplier. This creates an interesting network visualisation with supplier nodes revolving around agency nodes. Communities form more easily with one or several agencies at the center. Based on this observation, we've decided to separate our visualisation design into two distinct views: an overall network view and a community network view.
...
The network visualisation allows users to select date ranges and display methods to clearly see relationships between agencies and suppliers. Tooltips display important information such as node names, number of contracts, and total award amounts, helping users quickly understand key details about the connections they're viewing.
...
In this module, network community is the focus of the analysis. Two tabs are designed to illustrate a community network overview and zoom-in view for a focused community. The communities are distinguished by colours. When a user selects a community, the selection will sync to the **Community Explorer** tab. Users can then zoom in to view the community members and their information.
...
We want users to leverage different algorithms to best discover insights, so there are several options: Edge-betweenness, Louvain, Walktrap, etc. Each may produce different numbers of communities.
...
Network layouts in community detection algorithms can reveal different structural aspects of a network, thereby providing complementary insights for analysis.
...
Currently, the Network Analysis Module is developed into two sub-modules. The first provides a network overview for the user's selected date range. The second offers network community detection methods, visualisations, as well as metrics derived from the networks.

### 5.1 Network Overview

The Network Overview module consists of four main sections that provide comprehensive insights into Singapore agency procurement dynamics:

The first section features a Network visualisation board with summary information, offering users a high-level view of the entire procurement network and its basic metrics.

The second section presents detailed metrics about agencies, suppliers, and top contracts within the network. Users can leverage these metrics to extract data for further analysis.

The third section provides a focused Ego Network visualisation. When a user clicks on a node in the main network view (first section), they are directed to this section where an Ego Network is plotted along with relevant summary information.

The fourth section displays detailed metrics for the selected Ego Network, including specific information about agencies, suppliers, and connections within this focused network view.
...
The figure below illustrates the interactive features that will be implemented in the Overview page development. The primary objective of this page is to provide users with a comprehensive view of relationship patterns within the procurement network.

Users can refine their analysis by filtering agencies according to agency type. Additionally, they can focus on specific procurement categories to examine targeted network relationships. These interactive filtering capabilities enable more precise and relevant network analysis based on user requirements.
...
This is the proposed detailed metrics table for the network displayed in the first section. Within this interface, users can employ keyword searches to access specific data related to agencies, suppliers, and tenders. The table enables users to identify top-performing agencies, major suppliers, and significant contracts (ranked by award amount) within the network. Additionally, users can utilize the data sorting feature to organise information according to their analytical needs.
...
This section provides users with an exclusive view of the selected node and all its direct connections, creating what is known as an ego-centric network visualisation. The focal node (ego) is distinctly represented as a star shape. Connected nodes are differentiated by shape: suppliers appear as dots, whilst agencies are displayed as diamonds.

Node labels are displayed by default for easy identification. Interactive functionality allows users to access detailed information by hovering over nodes and edges. Importantly, the metrics displayed for each node are not limited to the ego-centric network view but are calculated based on the node's position within the entire selected network dataset.

![](images/Ms-section4-01.png){fig-align="center"}

### Section Four (Ego Metrics)

This section displays a metric table specifically for the Ego Network. It maintains the same design as the main network metric table but presents a more focused dataset derived from the Ego Network visualisation. The decision to separate this table from the visualisation was made to optimise user experience, preventing users from having to scroll down an excessively long page to view the tabular data.
...
In this Network Overview module, the application comprises 3 main sections, focusing exclusively on network community analysis. The design rationale employs colour-coded visualisation to distinguish communities identified by algorithmic analysis. The module is organised into three dedicated sections:

The first section provides an overview of network communities, featuring a colour-coded visualisation, basic metrics, and a bar chart representation of community distribution.

The second section displays the Ego community selected by the user, prominently revealing important metrics at the top of the page for immediate insight.

The third section presents a metric table for the selected Ego community, with details on each node, node type, and various centrality metrics for comprehensive analysis.
...
The figure below illustrates the interactive features planned for the overview page development. The primary objective is to provide users with a comprehensive understanding of community distribution within the selected data range. Users can utilise the control panel to define their analytical dataset by specifying parameters such as date range, agency type, specific agencies, suppliers and tender type.

Additionally, users can adjust network visualisation settings to control the density of displayed edges, select preferred network layouts and choose from various community detection methods based on their analytical requirements.

A synchronised view functionality will be implemented between the main overview and the ***Community Explorer*** tab. When users select a community using the drop-down menu to highlight it in the main view, they will simultaneously be able to examine a focused view of the selected community in the ***Community Explorer*** tab.
...
Hoffman, M. (2021). Methods for Network Analysis. In Social Network Analysis. <https://bookdown.org/markhoff/social_network_analysis/network-visualization-and-aesthetics.html>

Jayawickrama, T. D. (2021, January 29). Community detection algorithms. Medium. <https://medium.com/data-science/community-detection-algorithms-9bd8951e7dae>
